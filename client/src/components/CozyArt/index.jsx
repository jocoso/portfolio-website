...
const CozyArt = ({
    ...
    // Function to get the logo URI with fallback
    const getLogoUri = () => {
        return image ? `/assets/${image}` : "https://via.placeholder.com/150";
    };
    ...
    return (
        <div
            className={`${className} 
                ...
                rounded-full overflow-hidden border-8 
                hover:border-accent transition-all 
                duration-300 ease-in-out cursor-pointer`}
            onMouseEnter={onMouseEnter}
            onMouseLeave={onMouseLeave}
            onClick={onClick}
            aria-label={title || "Artwork"} // Accessibility enhancement
        ...
        >
            <CozyImage
                ...
                uri={getLogoUri()}
                alt={title || "Artwork image"} // Fallback alt text
            />
        </div>
    );
};
